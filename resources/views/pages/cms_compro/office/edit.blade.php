        ...
        <section class="content">
          ...
          <div class="box box-primary">
                <div class="box-header with-border">
                  <h3 class="box-title">@lang('main.edit_office')</h3>
                </div>

                <form action="{{ route('office.update', ['id' => $id]) }}" method="post" enctype="multipart/form-data">
                  {{ csrf_field() }}
                  <div class="box-body">
                  ...
                  @if (Session::has('success'))
                    ...
                    <div class="form-group">
                        ...
                        <select name="dir" id="dir" class="form-control{{ $errors->has('dir') ? ' is-invalid' : '' }}" required>
                            ...
                            @foreach($dirs as $dir)
                            <option value="{{$dir->directory_id}}" <?php if($dir->directory_id === $d->directory_id){echo "selected";}else{}?>>{{$dir->directory}}</option>
                            @endforeach
                        </select>
                    </div>

                    <div class="form-group">
                        <label for="country">@lang('main.nation')</label>
                        <input type="text" class="form-control{{ $errors->has('website') ? ' is-invalid' : '' }}" name="country" id="country" placeholder="Negara" value="{{ $d->country }}" required>
                    </div>


                    <div class="form-group">
                        <label for="website">@lang('main.web_address')</label>
                        <input type="text" class="form-control{{ $errors->has('website') ? ' is-invalid' : '' }}" name="website" id="website" placeholder="Alamat Website" value="{{ $d->website }}">
