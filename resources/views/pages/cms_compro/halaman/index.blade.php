...
<div class="content-wrapper">
        ...
        <section class="content-header">
          ...
          <ol class="breadcrumb">
            ...
            <li class="active">Data Menu Header</li>
          </ol>
        </section>
        ...
        <section class="content">
          ...
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title">Data Menu Header</h3>
            ...
            <div class="box-body">
                    ...
                    <table id="example1" class="table table-bordered table-striped">
                      ...
                      <tr>
                        <td>{{ $no++ }}</td>
                        <td><a href="{{ URL::to('/').'/halaman/'.$post->id.'/'.$post->url }}" target="_blank">{{ URL::to('/').'/halaman/'.$post->id.'/'.$post->url }}</a></td>
                        <td>{{ $post->name_in }}</td>
                        <td>{{ $post->title_in }}</td>
                        <td>{{ $post->name }}</td>
                        <td>{{ strip_tags( shorter($post->content_in) ) }}</td>
                        ...
                        <td>
                            <a href="{{ route('hal.edit', ['id' => $post->id]) }}">
                                <i class="fa fa-edit text-blue" style="font-size: 20px"></i>
                            </a>&nbsp;
                            <a href="{{ route('hal.delete', ['id' => $post->id]) }}" onclick="return confirm('Apakah anda yakin akan menghapus {{ $post->title_in }}?')">
