    ...
    <div  class="row">
      <div class="col-md-12">
        ...
        <div class="contact_form gray-bg">
          <form method="POST" action="{{ route('register.verifying') }}">
            @csrf
            <input type="hidden" name="email" value="{{ $email }}">
            <div class="form-group">
                <div class="row">
                    <div class="col-6">
                        <label class="control-label">Please enter verification code</span></label>
                          <input type="text" class="form-control {{ $errors->has('token') ? ' is-invalid' : '' }} white_bg" name="token" value="{{ @Input::get('token') ?? old('token') }}" autofocus placeholder="Verification Code">
                          @if ($errors->has('token'))
            ...
            @if(@Input::get('notValid') == 1)
                <div class="form-group" style="color: #dc3545;">
                  Verification Code was not a valid one, please resend the code
                </div>
            @endif

            @if(@Input::get('error') == 1)
                <div class="form-group" style="color: #dc3545;">
                  Something error happened, please contact our administrator, or resend again the code
                </div>
            @endif

            <div class="form-group">
              Not see the email? <a href="{{url('/')}}/resend-verify/{{ $email }}">Click here to resend verification code</a>
            </div>
