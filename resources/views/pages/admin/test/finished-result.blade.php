                ...
                <div class="card-body">
                    <table style="width: 50%;" class="table table-striped table-bordered table-detail">
                      <tr>
                        ...
                        <td width="5%">:</td>
                        <td>{{ @\App\User::whereId($user_id)->first()->name }}</td>
                      </tr>
                      <tr>
                    ...
                    <div style="padding: 10px; margin-bottom: 30px;">
                        ...
                        <form id="add_form" action="{{ route('online.test.finished.result') }}" method="post" enctype="multipart/form-data">
                                ...
                                <div style="float: left; width: {{ (100/(@$data->column ?? 1))  }}%;">
                                    ...
                                    @elseif(@$data->type == 'Question')
                                        ...
                                        @if(@$data->answer_type == 'Upload Files')
                                          ...
                                          <?php
                                            $uploaded = FileAttached::whereCode(@$data->code)->whereCareerId(@$career_id)->whereUserId(@$user_id)->get();
                                          ?>
                                        ...
                                        @if(@$data->answer_type == 'Plain')
                                            ...
                                            <?php
                                              $answer = Answer::whereCode(@$data->code)->whereCareerId(@$career_id)->whereUserId(@$user_id)->first();
                                            ?>
                                        ...
                                        @if(@$data->answer_type == 'Choices')
                                            ...
                                            <?php
                                                ...
                                                $isRightAnswer = @$data->right_answer == @$answer->answer;
                                                $isTableText = strpos(@$question_answer->text, '</table>') !== false;
                                            ?>

                                            @if(@$question_answer)
                                              <div style="float:left; margin-top: 8px !important; font-weight: 600; {{ $isRightAnswer ? 'color: #00B9B5;' : 'color: red;' }}">Answer: {{ @$data->right_answer }} {!! (@$question_answer && !$isTableText) ? ' - '.@$question_answer->text : '' !!}</div>
                                            @endif
                                                            ...
                                                            <td style="text-align: center;">
                                                                @if(@$data->group_answer_type == '1')
                                                                    ...
                                                                    @if($column->disabled != 1)
                                                                        ...
                                                                        <?php
                                                                          $answer = Answer::whereCode(@$column->code)->whereUserId(@$user_id)->first();
                                                                        ?>
                                                                ...
                                                                @else
                                                                    ...
                                                                    <?php
                                                                      ...
                                                                      if(@$data->transpose_group == 2){
                                                                        ...
                                                                        $answer = Answer::whereCode(@$row->group_code)->whereUserId(@$user_id)->first();

                                                                        if(@$answer->answer == @$column->text) $checked = true;

                                                                      }else{

                                                                        $answer = Answer::whereCode(@$column->group_code)->whereUserId(@$user_id)->first();
                            ...
                            <div class="box-footer" style="position: absolute; bottom: 20px; right: 30px;">
                                ...
                                <a style="padding-left: 20px; padding-right: 20px;" class="btn btn-success" href="{{ route('result.index') }}">Back to Result</a>
