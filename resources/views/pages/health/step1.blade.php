...
<div style="float:right;">
  ...
  <div class="tab">
    ...
    <p class="tengah"><span style="font-size: 16px; font-weight: bold">Adalah riwayat penyakit yang pernah diderita. Mohon diberi tanda pada setiap penyakit dibawah ini untuk jawaban YA atau TIDAK serta beri penjelasan pada pertanyaan Nomor 2 dengan benar, jujur dan bertanggung jawab.</span></p>
    ...
    <div class="row">
        ...
        <input type="hidden" name="q[{{$dhealth->id_health}}]" value="{{$dhealth->id_health}}">
            <div class="col-sm-3">
                ...
                <span style="font-size: 12px">{!!$dhealth->name!!}</span>
            </div>
            <div class="col-sm-1">
                    <span style="font-size: 8px">
                        <input type="radio" name="q_{{$dhealth->id_health}}" value="Y">Ya <input type="radio" name="q_{{$dhealth->id_health}}" value="T">Tidak
                    </span>
            </div>
        @endforeach
    </div>
    ...
    <div class="row">
        ...
        <div class="col-sm-2" style="height: 50px">
            <?php $rand = rand(1,2); ?>
            <input type="radio" name="q_{{$dhealth2->id_health}}" value="Y">Ya <input type="radio" name="q_{{$dhealth2->id_health}}" value="T">Tidak
        </div>
        <div class="col-sm-3" style="height: 50px">
            <?php if($dhealth2->id_health != 49 && $dhealth2->id_health != 50 && $dhealth2->id_health != 60 && $dhealth2->id_health != 61 && $dhealth2->id_health != 63 && $dhealth2->id_health != 64 && $dhealth2->id_health != 65) { ?>
                <input type="text" name="k_{{$dhealth2->id_health}}" value="" class="input" placeholder="Jika Ya , Jelaskan...">
            <?php } ?>
            <?php if($dhealth2->id_health == 63) { ?>
                <input type="text" name="k_{{$dhealth2->id_health}}" value="" class="input" placeholder="Berapa">
            <?php } ?>
        </div>
    ...
    <div class="row">
            ...
            <div class="col-sm-2" style="height: 50px">
                <input type="radio" name="q_{{$dhealth3->id_health}}" value="Y">Ya <input type="radio" name="q_{{$dhealth3->id_health}}" value="T">Tidak
            </div>
            <div class="col-sm-3" style="height: 50px">
                <?php if($dhealth3->id_health == 68) { ?>
                    <input type="text" name="k_{{$dhealth3->id_health}}" value="" class="input" placeholder="Jika Ya , Sudah berapa lama...">
                <?php } ?>
            </div>
            <?php $no++; ?>
            @endforeach
