        ...
        <div class="row">
            <div class="col-sm-3">
                <div class="nav-services flex-box">
                    ...
                    <a href="{{ route('compro.pubDB') }}" class="active">@lang('main.publication_link5')</a>
                </div>
            </div>
            <div class="col-sm-9">
			<div class="row">
				...
				<div class="col-sm-3 wrap-pub-download">
					...
					<h6>
					    ...
					    <?php $lang_org = session('language');
                         if($lang_org=='en')
                         {
                          echo $produk[$i]->title_en;
                          }
                         else
                          {
                           echo $produk[$i]->title_in;
                           }
                                			    
                          ?>
					    </h6>
						<img class="img-pub"  src="{{asset($produk[$i]->img)}}"
							alt="">
						<div>
						<p>
                        <?php $lang_org = session('language');
                         if($lang_org=='en')
                         {
                          echo $produk[$i]->text_en;
                          }
                         else
                          {
                           echo $produk[$i]->text_in;
                           }
                                			    
                          ?>
						    </p>
						</div>
						<a href="{{ asset($produk[$i]->doc) }}"
							class="btn btn-warning">@lang('main.publication_download')</a>
					</div>
				</div>
			<?php } ?>
